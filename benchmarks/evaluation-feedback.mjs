...
for (const sample of corpus) {
  ...
  const result = { providerResultId: `provider-${sample.id}`, providerId: "fixture", criterionId: "criterion", runId: run.id, resultDigest: `result-${sample.id}`, recordedAt: "2026-09-04T00:00:01.000Z", provenance: "verified_provider", result: { criterionId: "criterion", outcome: sample.outcome, evidence: sample.outcome === "inconclusive" ? [] : [{ provider: "fixture", digest: `sha256-${sample.id}`, targetStateRef: run.repositoryStateRef, observedAt: "2026-09-04T00:00:01.000Z" }], reasonCode: sample.reasonCode } };
