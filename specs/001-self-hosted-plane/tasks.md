...
- [X] T032 [US3] Run the official MCP Apps basic-host/conformance route and one real-host probe; record compatibility evidence without changing the core protocol era. The in-memory contract passes, the rebuilt live stdio server exposes readable `ui://my-pi/graph` and `ui://my-pi/theater` resources with MIME `text/html;profile=mcp-app` against a ready coordination daemon, and the installed OpenCode CLI connects to my-pi as a real host. Baseline remains 13 tools; live coordination opt-in exposes 19 tools. Host-side MCP Apps rendering is not claimed without direct UI evidence, so qualification remains withheld in `evidence/track-c-mcp-apps-2026-09-13.json` (sha256:34d99d02e43e8ed5db2f5b81b071525bdcf7ff604eb7df68f6e6dbdb7d0e2341; candidate `1b1adf20991f7d3e059d63d26ab379e7e60e300b`).
